...
namespace NURU
{
    ...
    {
        NURU_CORE_INFO("Loading mesh file at: {0}.", path);
        ...
        {
            NURU_CORE_ERROR("Assimp failed to load model at path: {0}", path);   
            return nullptr;
        }
    ...
    // --------------------------------------------------------------------------------------------
    Mesh* MeshLoader::parseMesh(aiMesh* aMesh, const aiScene* aScene, Vec3& out_Min, Vec3& out_Max)
    {
        ...
        {
            positions[i] = Vec3(aMesh->mVertices[i].x, aMesh->mVertices[i].y, aMesh->mVertices[i].z);
            normals[i] = Vec3(aMesh->mNormals[i].x, aMesh->mNormals[i].y, aMesh->mNormals[i].z);
            if (aMesh->mTextureCoords[0])
            {
                uv[i] = Vec2(aMesh->mTextureCoords[0][i].x, aMesh->mTextureCoords[0][i].y);

            }
            if (aMesh->mTangents)
            {
                tangents[i] = Vec3(aMesh->mTangents[i].x, aMesh->mTangents[i].y, aMesh->mTangents[i].z);
                bitangents[i] = Vec3(aMesh->mBitangents[i].x, aMesh->mBitangents[i].y, aMesh->mBitangents[i].z);
            }
