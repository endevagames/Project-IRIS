...
namespace NURU
{
    ...
    void Renderer::Init()
    {
        ...
        {
            NURU_CORE_INFO("Failed to initialize GLAD");
            return;
        }          
        else
        {
            NURU_CORE_INFO("OpenGL functions succesfully loaded.");
            NURU_CORE_INFO("Version - Major: {0} Minor: {1}", std::to_string(GLVersion.major), std::to_string(GLVersion.minor));
            NURU_CORE_INFO("Driver: {0} GPU: {1}" , std::string((char*)glGetString(GL_VENDOR)), std::string((char*)glGetString(GL_RENDERER)));
        }
    ...
    void Renderer::PushRender(SceneNode* node)
    {
        ...
        while (!nodeStack.empty())
        {
            ...
            if (node->Mesh)
            {
                Vec3 boxMinWorld = node->GetWorldPosition() + (node->GetWorldScale() * node->BoxMin);
                Vec3 boxMaxWorld = node->GetWorldPosition() + (node->GetWorldScale() * node->BoxMax);
        ...
        */
        ...
        for (auto it = m_PointLights.begin(); it != m_PointLights.end(); ++it)
        {
            if ((*it)->RenderMesh)
            {
                ...
                Mat4 model;
                NURU::Translate(model, (*it)->Position);
    ...
    // ------------------------------------------------------------------------
    void Renderer::AddIrradianceProbe(Vec3 position, float radius)
    {
        m_ProbeSpatials.push_back(Vec4(position, radius));
    }
    // ------------------------------------------------------------------------
    ...
    void Renderer::renderCustomCommand(RenderCommand* command, Camera* customCamera, bool updateGLSettings)
    {
        ...
        for (auto it = uniforms->begin(); it != uniforms->end(); ++it)
        {
            ...
            case SHADER_TYPE_MAT4:
                ...
                break;
            default:
                NURU_CORE_ERROR("Unrecognized Uniform type set.");
                break;
            }
        }
    ...
    void Renderer::renderToCubemap(std::vector<RenderCommand>& renderCommands, TextureCube* target, Vec3 position, unsigned int mipLevel)
    {
        ...
        for (unsigned int i = 0; i < 6; ++i)
        {
            ...
            camera->SetPerspective(NURU::Deg2Rad(90.0f), width / height, 0.1f, 100.0f);
    ...
    void Renderer::updateGlobalUBOs()
    {
        ...
        glBufferSubData(GL_UNIFORM_BUFFER,   0, sizeof(Mat4), &(m_Camera->Projection * m_Camera->View)[0][0]); // sizeof(Mat4) = 64 bytes
        glBufferSubData(GL_UNIFORM_BUFFER,  64, sizeof(Mat4), &m_PrevViewProjection[0][0]); 
        glBufferSubData(GL_UNIFORM_BUFFER, 128, sizeof(Mat4), &m_Camera->Projection[0][0]);
        glBufferSubData(GL_UNIFORM_BUFFER, 192, sizeof(Mat4), &m_Camera->View[0][0]);
        glBufferSubData(GL_UNIFORM_BUFFER, 256, sizeof(Mat4), &m_Camera->View[0][0]); // TODO: make inv function in math library
        // scene data
        glBufferSubData(GL_UNIFORM_BUFFER, 320, sizeof(Vec4), &m_Camera->Position[0]);
        // lighting
        unsigned int stride = 2 * sizeof(Vec4);
        for (unsigned int i = 0; i < m_DirectionalLights.size() && i < 4; ++i) // no more than 4 directional lights
        {
            glBufferSubData(GL_UNIFORM_BUFFER, 336 + i * stride,                      sizeof(Vec4), &m_DirectionalLights[i]->Direction[0]);
            glBufferSubData(GL_UNIFORM_BUFFER, 336 + i * stride + sizeof(Vec4), sizeof(Vec4), &m_DirectionalLights[i]->Color[0]);
        }
        for (unsigned int i = 0; i < m_PointLights.size() && i < 8; ++i) //  constrained to max 8 point lights in forward context
        {
            glBufferSubData(GL_UNIFORM_BUFFER, 464 + i * stride,                      sizeof(Vec4), &m_PointLights[i]->Position[0]);
            glBufferSubData(GL_UNIFORM_BUFFER, 464 + i * stride + sizeof(Vec4), sizeof(Vec4), &m_PointLights[i]->Color[0]);
        }
    }
    // --------------------------------------------------------------------------------------------
        ...
        if (IrradianceGI && irradianceProbes.size() > 0)
        {
            ...
            for (int i = 0; i < irradianceProbes.size(); ++i)
            {
                ...
                if (m_Camera->Frustum.Intersect(probe->Position, probe->Radius))
                {
                    ...
                    Mat4 model;
                    NURU::Translate(model, probe->Position);
    ...
    void Renderer::renderDeferredDirLight(DirectionalLight* light)
    {
        ...
        dirShader->SetVector("lightColor", NURU::Normalize(light->Color) * light->Intensity); 
    ...
    void Renderer::renderDeferredPointLight(PointLight* light)
    {
        ...
        pointShader->SetVector("lightColor", NURU::Normalize(light->Color) * light->Intensity);

        Mat4 model;
        NURU::Translate(model, light->Position);
